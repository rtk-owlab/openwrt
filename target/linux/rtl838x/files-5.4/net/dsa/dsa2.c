...
static int dsa_switch_setup(struct dsa_switch *ds)
{
	...
	if (!ds->devlink)
		return -ENOMEM;
	printk("In dsa_switch_setup A\n");
	err = devlink_register(ds->devlink, ds->dev);
	if (err)
		goto free_devlink;
	printk("In dsa_switch_setup B\n");
	err = dsa_switch_register_notifier(ds);
	if (err)
		goto unregister_devlink;
	printk("In dsa_switch_setup C\n");
	err = ds->ops->setup(ds);
	if (err < 0)
		goto unregister_notifier;

	printk("slave bus %x ops read: %x\n", (u32)(ds->slave_mii_bus), (u32) (ds->ops->phy_read));
